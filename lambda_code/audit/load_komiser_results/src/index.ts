...
/** Metadata only — aggregate lee el JSON Komiser desde S3 (evita DataLimitExceeded). */
export const handler: Handler<Input> = async (event) => {
  ...
  const key =
    process.env['KOMISER_FINDINGS_KEY_TEMPLATE']?.replace('{tenantId}', event.tenantId).replace(
      '{auditId}',
      event.auditId,
  ...
  try {
    ...
    console.error('load_komiser_results failed', { key, message });
    return {
      engine: 'komiser-fargate',
      sourceBucket: bucket,
      sourceKey: key,
